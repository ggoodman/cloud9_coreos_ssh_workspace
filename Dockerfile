...
RUN \
  apt-get update && \
  apt-get install -y build-essential openssh-server sudo

RUN mkdir -p /var/run/sshd

# Create the core user with same uid:gid as host user
RUN groupadd -g $GID core
RUN useradd -g $GID -u $UID -d /home/core -m -s /bin/bash core
...
RUN tar -xvzf /tmp/etcdctl.tar.gz -C /usr/local/bin --strip-components=1 etcd-${ETCD_VERSION}-linux-amd64/etcdctl
RUN rm -rf /tmp/etcdctl.tar.gz
...
RUN tar -xvzf /tmp/fleetctl.tar.gz -C /usr/local/bin --strip-components=1 fleet-${FLEET_VERSION}-linux-amd64/fleetctl
RUN rm -rf /tmp/fleetctl.tar.gz
...
RUN bash -c '. ~/.nvm/nvm.sh && nvm install $NODE_VERSION && nvm use $NODE_VERSION && nvm alias default $(nvm version $NODE_VERSION)'
...
RUN adduser core sudo
RUN echo "core ALL=NOPASSWD: ALL" >> /etc/sudoers
